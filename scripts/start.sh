...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo "❌ API Server failed to start within 30 seconds. Check logs/api.log for details."
        echo ""
...
STREAMLIT_SERVER_HEADLESS=true streamlit run ui.py --server.headless true > logs/ui.log 2>&1 &
UI_PID=$!
